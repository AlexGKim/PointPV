...
def rg_coarsen_all(
    ...
    for level_idx in range(1, tree.depth + 1):
        ...
        for li, lj, node in pair_updates:
            ...
            d = u_cur[li] - u_cur[lj]
            logL_acc += -0.5 * (np.log(c_dd) + d**2 / c_dd) + np.log(2.0)
    ...
    if (stop_size > 1 or early_stop) and len(u_cur) > 1:
        if return_diagnostics:
            ...
            return float(logL_acc), C_cur.copy(), u_cur.copy(), level_sizes
        return float(logL_acc), C_cur.copy(), u_cur.copy()
    ...
    logL_acc += -0.5 * (np.log(c_final) + u_cur[0]**2 / c_final)
...
def rg_step_n2(u: np.ndarray, C: np.ndarray) -> float:
    ...
    log_det_s = float(np.log(c_ss_new))
