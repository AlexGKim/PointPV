#!/usr/bin/env python
"""
Run the MLF baseline log-likelihood scan over fsigma8.

Usage
-----
    python scripts/run_baseline.py --n 1000 --backend scipy
    python scripts/run_baseline.py --catalog data/mock_1000.npz --output results/baseline_1000.npz
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="MLF baseline fsigma8 scan")
    ...
    p.add_argument("--output", default=None,
                   help="Output .npz path.  Default: results/baseline_{n}.npz")
    p.add_argument("--backend", default=None,
                   choices=["scipy", "cupy"],
                   help="Sparse/linear-algebra backend (overrides POINTPV_BACKEND)")
...
def main() -> None:
    ...
    print(f"Running MLF scan over {args.n_grid} fsigma8 values ...")
    results = scan_fsigma8(u, catalog, fsigma8_values=fsigma8_values,
    ...
    if output is None:
        ...
        output = f"results/baseline_{args.n}.npz"
    np.savez(output, method="baseline", n=args.n, **results)
